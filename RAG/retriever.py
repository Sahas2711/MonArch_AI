...
def reciprocal_rank_fusion(ranked_lists: list[list], k: int = 60, top_n: int = 3) -> list:
    """Merge several ranked doc lists into one ranking via Reciprocal Rank Fusion."""
    scores: dict[str, float] = {}
    doc_by_key: dict[str, object] = {}

    for docs in ranked_lists:
        for rank, doc in enumerate(docs):
            key = doc.page_content  # dedupe identical chunks across retrievers
...
def build_bm25_retriever(documents: list, k: int = 3) -> Optional[BM25Retriever]:
    """Build BM25 keyword retriever from a list of documents."""
    if not documents:
        return None
